...
def generate_llm_summary(csv_file="../../azure_compute_usage_6months.csv", output_file="azure_cost_summary.json"):
    ...
    if len(underutilized) > 0:
        optimization["underutilized_vms"] = {
            ...
            "potential_savings": round(underutilized['cost_usd'].sum() * 0.5, 2),  # Assume 50% savings from right-sizing
    ...
    top_resources = df.groupby(['resource_name', 'resource_type', 'sku']).agg({
        'cost_usd': 'sum',
    ...
    print(f"  Ready to feed to LLM for analysis!")
