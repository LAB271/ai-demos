...
def validate_all_requests(df: pd.DataFrame) -> pd.DataFrame:
    ...
    validation_results = df.apply(
        lambda row: validate_request(row['Role'], row['SystemAccessRequested']),
        axis=1
    )
...
def generate_validation_report(df: pd.DataFrame):
    ...
    if rejected > 0:
        ...
        print(rejected_df[['RequestID', 'EmployeeID', 'EmployeeName', 'Role', 'SystemAccessRequested', 'ValidationReason']].head(sample_size).to_string(index=False))
    ...
    if high_risk > 0:
        ...
        print(high_risk_df[['RequestID', 'EmployeeID', 'EmployeeName', 'Role', 'SystemAccessRequested', 'ValidationReason']].head(sample_size).to_string(index=False))
    ...
    if rejected > 0:
        ...
        print("=" * 80)
        location_violations = rejected_df.groupby(['Location', 'Role', 'SystemAccessRequested']).size().reset_index(name='Count')
