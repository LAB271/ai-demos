...
class AnomalyLLMProcessor:
    ...
    def get_anomalies_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "time_range": {"start": self.df["timestamp"].min(), "end": self.df["timestamp"].max()},
            ...
            "top_anomaly_scores": anomalies.nlargest(5, "anomaly_score")[
                ["timestamp", "severity", "message", "anomaly_score"]
            ].to_dict("records"),
        }
...
Please be thorough but concise in your analysis."""
    ...
    def get_high_confidence_anomalies(self, score_threshold: float = -0.01) -> pd.DataFrame:
        ...
        high_confidence = self.df[(self.df["is_anomaly"]) & (self.df["anomaly_score"] <= score_threshold)].copy()
